...
class Runtime {
    ...
    public Runtime(Context context, ResourceUtil.ExpandedResource resource, DataLayer dataLayer, CustomFunctionCall.CustomEvaluator functionCallMacroEvaluator, CustomFunctionCall.CustomEvaluator functionCallTagEvaluator, EventInfoDistributor eventInfoDistributor) {
        ...
        CacheFactory.CacheSizeManager<ResourceUtil.ExpandedFunctionCall, ObjectAndStatic<TypeSystem.Value>> functionCacheSizeManager = new CacheFactory.CacheSizeManager<ResourceUtil.ExpandedFunctionCall, ObjectAndStatic<TypeSystem.Value>>() { // from class: com.google.tagmanager.Runtime.1
        ...
        this.mFunctionCallCache = new CacheFactory().createCache(1048576, functionCacheSizeManager);
        CacheFactory.CacheSizeManager<String, ObjectAndStatic<TypeSystem.Value>> macroCacheSizeManager = new CacheFactory.CacheSizeManager<String, ObjectAndStatic<TypeSystem.Value>>() { // from class: com.google.tagmanager.Runtime.2
            @Override // com.google.tagmanager.CacheFactory.CacheSizeManager
            public int sizeOf(String key, ObjectAndStatic<TypeSystem.Value> value) {
                return value.getObject().toByteArray().length + key.length();
            }
        };
        this.mMacroEvaluationCache = new CacheFactory().createCache(1048576, macroCacheSizeManager);
        ...
        for (ResourceUtil.ExpandedRule rule : this.mRules) {
            ...
            for (int i2 = 0; i2 < rule.getRemoveMacros().size(); i2++) {
                ...
                String ruleName2 = "Unknown";
                if (eventInfoDistributor.debugMode() && i2 < rule.getRemoveMacroRuleNames().size()) {
    ...
    ObjectAndStatic<Set<ResourceUtil.ExpandedFunctionCall>> calculateMacrosToRun(String macroName, Set<ResourceUtil.ExpandedRule> rules, final Map<ResourceUtil.ExpandedRule, List<ResourceUtil.ExpandedFunctionCall>> addMacros, final Map<ResourceUtil.ExpandedRule, List<String>> addMacroRuleNames, final Map<ResourceUtil.ExpandedRule, List<ResourceUtil.ExpandedFunctionCall>> removeMacros, final Map<ResourceUtil.ExpandedRule, List<String>> removeMacroRuleNames, Set<String> pendingMacroExpansions, RuleEvaluationStepInfoBuilder debugRulesEvaluation) {
        return calculateGenericToRun(rules, pendingMacroExpansions, new AddRemoveSetPopulator() { // from class: com.google.tagmanager.Runtime.3
            ...
            public void rulePassed(ResourceUtil.ExpandedRule rule, Set<ResourceUtil.ExpandedFunctionCall> add, Set<ResourceUtil.ExpandedFunctionCall> remove, ResolvedRuleBuilder debugRuleBuilder) {
                List<ResourceUtil.ExpandedFunctionCall> thisAddMacro = (List) addMacros.get(rule);
                List<String> thisMacroEnablingRuleNames = (List) addMacroRuleNames.get(rule);
                ...
                List<ResourceUtil.ExpandedFunctionCall> thisRemoveMacro = (List) removeMacros.get(rule);
                List<String> thisRemoveMacroRuleNames = (List) removeMacroRuleNames.get(rule);
    ...
    ObjectAndStatic<Set<ResourceUtil.ExpandedFunctionCall>> calculateTagsToRun(Set<ResourceUtil.ExpandedRule> rules, RuleEvaluationStepInfoBuilder debugRulesEvaluation) {
        return calculateGenericToRun(rules, new HashSet(), new AddRemoveSetPopulator() { // from class: com.google.tagmanager.Runtime.4
            @Override // com.google.tagmanager.Runtime.AddRemoveSetPopulator
            public void rulePassed(ResourceUtil.ExpandedRule rule, Set<ResourceUtil.ExpandedFunctionCall> add, Set<ResourceUtil.ExpandedFunctionCall> remove, ResolvedRuleBuilder debugRuleBuilder) {
    ...
    ObjectAndStatic<Boolean> evaluatePredicatesInRule(ResourceUtil.ExpandedRule rule, Set<String> pendingMacroExpansions, ResolvedRuleBuilder debugRuleBuilder) {
        ...
        for (ResourceUtil.ExpandedFunctionCall f2 : rule.getPositivePredicates()) {
            ...
            allStatic = allStatic && result2.isStatic();
        }
        debugRuleBuilder.setValue(Types.objectToValue(true));
        return new ObjectAndStatic<>(true, allStatic);
    }
    ...
    private ObjectAndStatic<TypeSystem.Value> evaluateMacroReferenceCycleDetection(String macroName, Set<String> pendingMacroExpansions, MacroEvaluationInfoBuilder debugMacroEvaluation) {
        ...
        if (cachedResult == null || this.eventInfoDistributor.debugMode()) {
            ...
            if (macrosToRun.getObject().isEmpty()) {
                ...
                ResourceUtil.ExpandedFunctionCall macro2 = macrosToRun.getObject().iterator().next();
                macro = macro2;
            }
            if (macro == null) {
                return DEFAULT_VALUE_AND_STATIC;
            ...
            ObjectAndStatic<TypeSystem.Value> toReturn = macroResult == DEFAULT_VALUE_AND_STATIC ? DEFAULT_VALUE_AND_STATIC : new ObjectAndStatic<>(macroResult.getObject(), isStatic);
    ...
    private ObjectAndStatic<TypeSystem.Value> macroExpandValue(TypeSystem.Value value, Set<String> pendingMacroExpansions, ValueBuilder debugValueBuilder) {
        ...
        switch (value.getType()) {
            ...
            case MACRO_REFERENCE:
                ...
                ObjectAndStatic<TypeSystem.Value> result = evaluateMacroReferenceCycleDetection(value.getMacroReference(), pendingMacroExpansions, debugValueBuilder.createValueMacroEvaluationInfoExtension());
                ObjectAndStatic<TypeSystem.Value> result2 = ValueEscapeUtil.applyEscapings(result, value.getEscapingList());
    ...
    private ObjectAndStatic<TypeSystem.Value> executeFunction(Map<String, FunctionCallImplementation> implMap, ResourceUtil.ExpandedFunctionCall functionCall, Set<String> pendingMacroExpansions, ResolvedFunctionCallBuilder debugFunctionCall) {
        ...
        if (cachedResult == null || this.eventInfoDistributor.debugMode()) {
            ...
            for (Map.Entry<String, TypeSystem.Value> originalParam : functionCall.getProperties().entrySet()) {
                ...
                ObjectAndStatic<TypeSystem.Value> result = macroExpandValue(originalParam.getValue(), pendingMacroExpansions, debugProperty.createPropertyValueBuilder(originalParam.getValue()));
