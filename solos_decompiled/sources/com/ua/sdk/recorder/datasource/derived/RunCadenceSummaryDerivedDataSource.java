...
public class RunCadenceSummaryDerivedDataSource extends DerivedDataSource {
    ...
    protected RunCadenceSummaryDerivedDataSource(DataSourceIdentifier dataSourceIdentifier, List<DataTypeRef> dataTypeRefs) {
        super(dataSourceIdentifier, dataTypeRefs);
        this.dataTypeRefTriggers = new ArrayList(Arrays.asList(BaseDataTypes.TYPE_RUN_CADENCE.getRef()));
    ...
    public void deriveDataPoint(RecorderCalculator recorderCalculator, DataSourceIdentifier triggerDataSourceIdentifier, DataTypeRef triggerDataTypeRef, DataPoint triggerDataPoint) {
        if (this.dataTypeRefTriggers.contains(triggerDataTypeRef) && this.recordData) {
            ...
            DataField max = BaseDataTypes.FIELD_RUN_CADENCE_MAX;
            DataField min = BaseDataTypes.FIELD_RUN_CADENCE_MIN;
            DataField avg = BaseDataTypes.FIELD_RUN_CADENCE_AVG;
            Double current = triggerDataPoint.getValueDouble(BaseDataTypes.FIELD_RUN_CADENCE);
            ...
            recorderCalculator.updateDataPoint(this.dataSourceIdentifier, this.summaryDataPoint, BaseDataTypes.TYPE_RUN_CADENCE_SUMMARY.getRef());
        }
    }
