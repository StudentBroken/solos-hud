...
abstract class AbstractAppSpiCall extends AbstractSpiCall implements AppSpiCall {
    ...
    public boolean invoke(AppRequestData requestData) {
        ...
        String kind = HttpRequest.METHOD_POST.equals(httpRequest.method()) ? "Create" : "Update";
        Fabric.getLogger().d(Fabric.TAG, kind + " app request ID: " + httpRequest.header(AbstractSpiCall.HEADER_REQUEST_ID));
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest request, AppRequestData requestData) {
        ...
        if (requestData.icon != null) {
            ...
            try {
                ...
                request2.part(APP_ICON_HASH_PARAM, requestData.icon.hash).part(APP_ICON_DATA_PARAM, ICON_FILE_NAME, ICON_CONTENT_TYPE, is).part(APP_ICON_WIDTH_PARAM, Integer.valueOf(requestData.icon.width)).part(APP_ICON_HEIGHT_PARAM, Integer.valueOf(requestData.icon.height));
