...
public class ViewPager extends ViewGroup {
    ...
    private void completeScroll(boolean postEvents) {
        ...
        if (needPopulate) {
            setScrollingCacheEnabled(false);
            boolean wasScrolling = this.mScroller.isFinished() ? false : true;
            if (wasScrolling) {
                this.mScroller.abortAnimation();
                int oldX = getScrollX();
                int oldY = getScrollY();
        ...
        if (this.mPageMargin > 0 && this.mMarginDrawable != null && this.mItems.size() > 0 && this.mAdapter != null) {
            ...
            for (int pos = firstPos; pos < lastPos; pos++) {
                ...
                if (this.mPageMargin + drawAt > scrollX) {
                    this.mMarginDrawable.setBounds(Math.round(drawAt), this.mTopPageBounds, Math.round(this.mPageMargin + drawAt), this.mBottomPageBounds);
