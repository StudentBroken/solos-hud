...
public class ViewDragHelper {
    ...
    private static final Interpolator sInterpolator = new Interpolator() { // from class: android.support.wearable.view.drawer.ViewDragHelper.1
    ...
    private final Runnable mSetIdleRunnable = new Runnable() { // from class: android.support.wearable.view.drawer.ViewDragHelper.2
    ...
    private ViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        ...
        this.mWearEdgeSizePx = resources.getDimensionPixelSize(R.dimen.drawer_view_edge_size);
    ...
    public void refreshEdgeSize() {
        if (this.mIsUiModeTypeWatch) {
            int edgeSizePx = (int) (WATCH_EDGE_SIZE_RATIO * this.mParentView.getHeight());
            this.mEdgeSize = Math.min(edgeSizePx, this.mWearEdgeSizePx);
        } else {
            this.mEdgeSize = (int) ((20.0f * this.mDisplayDensity) + 0.5f);
        }
    }

    public void captureChildView(View childView, int activePointerId) {
        if (childView.getParent() != this.mParentView) {
            String strValueOf = String.valueOf(this.mParentView);
            throw new IllegalArgumentException(new StringBuilder(String.valueOf(strValueOf).length() + 95).append("captureChildView: parameter must be a descendant of the ViewDragHelper's tracked parent view (").append(strValueOf).append(")").toString());
        }
